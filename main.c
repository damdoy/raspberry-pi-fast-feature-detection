...
// void blur_grayscale_image_faster(image_grayscale_t *image_source, image_grayscale_t *image_dest, uint kernel_size);
...
void draw_circle(int x, int y, int radius, image_rgb_t *img);
...
void image_draw(image_rgb_t *img, char *framebuffer, uint framebuffer_width){
    ...
    for(int i = 0; i < CAMERA_RESOLUTION_Y; i++){
        for(int j = 0; j < CAMERA_RESOLUTION_X*4; j+=4){
            ...
            fbp[idx] = img->img[offset_data+2];
            fbp[idx+1] = img->img[offset_data+1];
            fbp[idx+2] = img->img[offset_data+0];
            fbp[idx+3] = 0;
            offset_data += 3;
        }
    }
...
void image_draw_grayscale(image_grayscale_t *img, char *framebuffer, uint framebuffer_width){
    ...
    for(int i = 0; i < img->height; i++){
        for(int j = 0; j < img->width*4; j+=4){
            int idx = i*framebuffer_width*4+j;
            fbp[idx] = img->img[offset_data];
            fbp[idx+1] = img->img[offset_data];
            fbp[idx+2] = img->img[offset_data];
            fbp[idx+3] = 0;
            offset_data++;
        }
    }
